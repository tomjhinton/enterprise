...
void main() {
  ...
  vec2 pc = vec2(cos(angle), sin(angle)) * log(len) + .5;


  vec2 rote = rotateUV(pc, vec2(.5), PI * vTime * .05);
  vec2 roteC = rotateUV(pc, vec2(.5), -PI * vTime * .05);
  ...
  color.r+=  cheapNoise(color, sin(vTime));
  color.g+=  cheapNoise(color, cos(vTime));
  color.b+=  cheapNoise(color, sin(vTime * .5));
